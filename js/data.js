...
export function getMeta(id) {
  return META[id] || { icon: '📌', color: '#6c63ff', bg: 'rgba(108,99,255,0.12)' };
}
export function defSlots(d) {
  return (d < 5 ? WD : WE).map((s) => ({ ...s }));
}

export async function setSlots(uid, d, arr) {
  await dbSave(uid, `sched:${d}`, arr);
}
...
export async function buildHabits(uid) {
  ...
  const allSlots = await Promise.all(Array.from({ length: 7 }, (_, d) => getSlots(uid, d)));
  ...
  for (let d = 0; d < 7; d++) {
    ...
    slots
      .filter((s) => TRACK_TYPES.includes(s.type) && s.id)
      .forEach((s) => {
        if (!seen.has(s.id)) {
          seen.add(s.id);
  ...
  const full = habits.map((h) => {
    ...
    return {
      ...
      bg: m.bg,
      tag: freq,
      tagLabel: freq === 'wd' ? 'mon–fri' : freq === 'we' ? 'weekends' : 'every day',
    };
  });
  ...
  const result = [];
  if (edH.length) {
    result.push({ group: true, label: 'Everyday Habits · Mon–Sun', color: '#20d68a' });
    result.push(...edH);
  }
  if (wdH.length) {
    result.push({ group: true, label: 'College / Weekdays · Mon–Fri', color: '#6c63ff' });
    result.push(...wdH);
  }
  if (weH.length) {
    result.push({ group: true, label: 'Weekend · Sat & Sun', color: '#ffbe3d' });
    result.push(...weH);
  }
  return result;
}
