...
async function setupFormulaVault(uid) {
  ...
  const renderFormulas = async () => {
    ...
    formulas.forEach((f, idx) => {
      ...
      row.style.cssText =
        'background:var(--surface); border:1px solid var(--border); border-radius:6px; padding:8px; display:flex; justify-content:space-between; align-items:center;';
      row.innerHTML = `
                <div style="flex:1;">
...
async function setupRevisionChecklist(uid) {
  ...
  const renderRev = async () => {
    ...
    revs.forEach((r, idx) => {
      const row = document.createElement('div');
      row.style.cssText =
        'display:flex; justify-content:space-between; align-items:center; font-size:12px; color:var(--text);';
...
async function setupWrongAnswerLog(uid) {
  ...
  const renderWA = async () => {
    ...
    if (logs.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="4" style="text-align:center; padding:12px; color:var(--dim); font-size:11px;">No mistakes logged yet. Keep solving!</td></tr>';
      return;
    }
  ...
  addBtn.onclick = async () => {
    ...
    logs.unshift({
      prob,
      desc,
      date: new Date().toLocaleDateString('default', { month: 'short', day: 'numeric' }),
    });
    await dbSave(uid, 'study:wa', logs);
