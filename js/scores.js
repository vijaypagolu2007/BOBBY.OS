...
export async function renderScoreTracker(uid, selectSemester = null) {
    ...
    if (semNames.length === 0) {
        ...
        subjectsContainer.innerHTML = '<div style="text-align:center; padding:12px; color:var(--dim); font-size:11px;">Add a semester to start tracking.</div>';
    ...
    if (subjects.length === 0) {
        subjectsContainer.innerHTML = '<div style="text-align:center; padding:12px; color:var(--dim); font-size:11px;">No courses logged for this semester.</div>';
    } else {
        subjects.forEach((sub, idx) => {
            const row = document.createElement('div');
            row.style.cssText = 'display:flex; justify-content:space-between; align-items:center; background:var(--surface); border:1px solid var(--border); border-radius:6px; padding:6px 10px; font-size:11px;';
            row.innerHTML = `
