import { initAuth, loginWithGoogle, doLogin, doRegister, doLogout, currentUser } from './auth.js';
...
async function init() {
  ...
  initAuth(async (user) => {
    ...
    if (dbStatus) {
      dbStatus.textContent = auth.onAuthStateChanged
        ? 'Connected to Cloud Persistence'
        : 'Running in Local-Only Mock Mode';
...
async function bootApp(user) {
  ...
  preloadAllUserData(user.uid).catch((err) =>
    console.error('BOBBY.OS: Preload background error:', err)
  );
  ...
  if (forceSyncBtn) {
    forceSyncBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        const activeTab =
          ['habit', 'power', 'sched', 'notes'].find((id) => {
    ...
    const doNlpAdd = async () => {
      ...
      for (const pat of rangePatterns) {
        ...
        if (m) {
          ...
          const sfx = ['evening', 'night', 'pm'].includes(lc)
            ? 'PM'
            : text.toLowerCase().includes('evening') || text.toLowerCase().includes('pm')
              ? 'PM'
              : 'AM';
