...
async function scheduleNativeDailyAlerts() {
    ...
    try {
        await LocalNotifications.cancel({ notifications: [{ id: HABIT_NOTI_ID }, { id: EXAM_NOTI_ID }] });
...
export async function checkAndTriggerExamAlert(uid, force = false) {
    ...
    try {
        ...
        const paperList = todaysExams.flatMap(exam =>
            (exam.papers || []).map(paper => `${paper.code || 'Exam'}${paper.slot ? ` (${paper.slot})` : ''}`)
        );
        const summary = paperList.length > 0 ? paperList.join(', ') : `${paperCount} paper${paperCount > 1 ? 's' : ''}`;
