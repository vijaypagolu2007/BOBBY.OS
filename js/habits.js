...
export async function renderHabits(uid) {
  ...
  ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].forEach((n, i) => {
    ...
    if (!th) return;
    const isT = iso(dates[i]) === tStr,
      isE = EXAM_D.has(iso(dates[i]));
  ...
  H.forEach((h) => {
    ...
    const s = streak(habitData, h.id, wk);
